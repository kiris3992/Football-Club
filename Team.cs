...
namespace LabFootball
{
    class Team
    {
        ...
        public static string PrintAllTeamsWithPlayers(List<Team> teams)
        {
            StringBuilder sb = new StringBuilder();

            foreach(Team team in teams)
            {
                sb.Append($"\n\nTeam ID: {team.ID} Name: {team.Name} Attack: {team.Attack}\n\tPlayers: {team.Players.Count}");
        ...
        public static List<Team> GetRandomTeamsToList(int totalTeams, int totalPermanentPlayers, int totalTransferPlayers)
        {
            ...
            for (int i = 0; i < randTeamNames.Count; i++)
            {
                ...
                while (randNames.Count < (totalPermanentPlayers + totalTransferPlayers))
                {
                ...
                for (int j = 0; j < randNames.Count; j++)
                {
                    ...
                    {
                        players.Add(new PlayerPermanent(j + 1, randNames[j], goals, rnd.Next(0, 81) + goals, dt));
                    }
                    else
                    {
                        players.Add(new PlayerTransfer(j + 1, randNames[j], goals, rnd.Next(0, 81) + goals, dt));
                    }
                }
